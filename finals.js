// questions.js
window.TEST_QUESTIONS = {
  mock1: [
    {
      q: "Qorincha devoridagi ko‘ndalang-targ‘il muskullar qaysi anatomik tuzilmalar bilan bog‘langan?",
      a: [
        "Xordae tendinea",
        "Moderator bog‘lama",
        "Qorinchalararo to‘siq",
        "Krista terminalis",
        "Sulkus terminalis",
      ],
      correct: 0,
    },
    {
      q: "Qo’shimcha Yarim toq Vena qaysi venaga quyiladi?",
      a: [
        "Azigos venaga",
        "Yarim toq Venaga",
        "Yuqori qovurg‘alararo venaga",
        "Brachiosefalik venaga",
        "O‘mrov osti venasiga",
      ],
      correct: 0,
    },
    {
      q: "Sindesmoz misoli?",
      a: [
        "Pastki tibiofibular va timpanostapedial",
        "Proksimal tibiofibular va radioulnar",
        "Intervertebral disklar",
        "Yelka bo‘g‘imi",
      ],
      correct: 0,
    },
    {
      q: "Skelet mushagi qoplamalari qanday?",
      a: [
        "Epimizium, perimizium, endomizium",
        "Perimizium, endomizium, epimizium",
        "Origin va insertion",
        "Sirkular va membranali",
      ],
      correct: 0,
    },
    {
      q: "Xanjarsimon o‘simtaning kaudal uchi qaysi tuzilma bilan birikadi?",
      a: [
        "Diafragma",
        "Jigarning yuqori yuzi",
        "Yurak pastki chegarasi",
        "Linea alba",
        "Ko‘krak suyagi tanasi",
      ],
      correct: 3,
    },
    {
      q: "Umurtqalararo diskning markaziy qismi qaysi embrional tuzilmandan rivojlanadi?",
      a: ["Neyral nay", "Mezoderma", "Notoxord", "Sklerotom", "Ektoderma"],
      correct: 2,
    },
    {
      q: "Auskultatsiya uchburchagining tubi qaysi muskul tomonidan hosil qilinadi?",
      a: [
        "Trapetsiyasimon muskul",
        "Orqaning keng muskuli",
        "Katta rombsimon muskul",
        "Kichik rombsimon muskul",
        "Umurtqani rostlovchi muskul",
      ],
      correct: 2,
    },
    {
      q: "Impulslarni hujayra tanasiga olib keluvchi nerv tolasi qaysi?",
      a: ["Akson", "Dentrit", "Sinaps", "Gangliy", "Neyrofibrilla"],
      correct: 1,
    },
    {
      q: "Limfatik tomirlarning marjon shodasiga o‘xshash ko‘rinishi nimaga bog‘liq?",
      a: [
        "Qalin devoriga",
        "Muskulli qatlamiga",
        "Klapanlar borligiga",
        "Nerv tolalariga",
        "Keng lumeniga",
      ],
      correct: 2,
    },
    {
      q: "Quyidagi tolalardan qaysi biri xolinergik hisoblanadi?",
      a: [
        "Simpatik postganglionar",
        "Simpatik preganglionar",
        "Adrenergik tolalar",
        "Arrektor pili tolalari",
        "Parasimpatik postganglionlar",
      ],
      correct: 1,
    },
    {
      q: "Harakat paytida ishqalanishni kamaytirib, sinovial suyuqlik bilan to‘liq yassi qopcha hosil qiladigan tuzilma qaysi?",
      a: [
        "Fatsiya",
        "Retinakulum",
        "Bursa",
        "Aponevroz",
        "Sinovial pay qinlari",
      ],
      correct: 2,
    },
    {
      q: "Bachadon nayi ampula qismida qaysi jarayon sodir bo‘ladi?",
      a: [
        "Ovulatsiya",
        "Fertilizatsiya",
        "Implantatsiya",
        "Ovogenez",
        "Menstruatsiya",
      ],
      correct: 1,
    },
    {
      q: "Quyidagi muskullardan qaysi biri chuqur qavat muskullariga kiradi va umurtqa pog‘onasini burish va bukish vazifasini bajaradi?",
      a: [
        "Splenius cervicis",
        "Longissimus",
        "Semispinalis",
        "Iliocostalis",
        "Levatores costarum",
      ],
      correct: 2,
    },
    {
      q: "Agar multifidus muskuli zaiflashsa va bel sohasida stabilizatsiya buzilsa, qaysi holat yuz berishi mumkin?",
      a: [
        "Bo‘yinning lateral bukilishi kuchayadi",
        "Bel og‘rig‘i va umurtqa pog‘onasi instabilizatsiyasi",
        "Qovurg‘alarni ko‘tarish qobiliyati oshadi",
        "Bosh va bo‘yinni yozish qobiliyati oshadi",
        "Segmentar muskullar kuchayadi",
      ],
      correct: 1,
    },
    {
      q: "Diafragmaning markaziy qismi sezuvchi tolalarni qaysi nervdan oladi?",
      a: [
        "Qovurg‘alararo nervlar",
        "Frenik nerv",
        "Torakal simpatik nervlar",
        "Vagus nerv",
        "Intercostal nervlar",
      ],
      correct: 1,
    },
    {
      q: "Perikard yemirilishi natijasida yuzaga keladigan patologik holat qaysi?",
      a: [
        "Diafragma falaji",
        "Yurak tamponadasi",
        "Papillar mushaklar qisqarishi",
        "Krista terminalis o‘zgarmasligi",
        "O‘ng qorinchada tromb hosil bo‘lishi",
      ],
      correct: 1,
    },
    {
      q: "37 yoshli bemorda plevral effuziya aniqlangan. Suyuqlikni chiqarish uchun toraksentez qilinadi. Ignani qaysi qovurg‘a oralig‘ida kiritish xavfsiz?",
      a: [
        "1 va 3-qovurg‘a",
        "3 va 5-qovurg‘a",
        "5 va 7-qovurg‘a",
        "7 va 9-qovurg‘a",
        "9 va 11-qovurg‘a",
      ],
      correct: 3,
    },
    {
      q: "Qaysi struktura o‘ng bo‘lmacha devorida yuqori kovak vena ochilishidan pastki kovak vena ochilishigacha uzatilgan mushak tizmasi bo‘lib, SA tugunining kelib chiqishini ta’minlaydi?",
      a: [
        "AV tutami",
        "Krista terminalis",
        "Pektinat mushaklar",
        "Purkinje tolalari",
        "AV tugun arteriyasi",
      ],
      correct: 1,
    },
    {
      q: "Qaysi plevra turi og‘riqqa kam sezgir, ammo nafas olish refleksi bilan bog‘liq vasomotor tolalarga sezgir hisoblanadi?",
      a: [
        "Parietal plevra",
        "Visseral plevra",
        "Diafragma plevrasi",
        "Qovurg‘a plevrasi",
        "Mediastinal plevra",
      ],
      correct: 1,
    },
    {
      q: "65 yoshli ayolda bo‘y qisqarishi, bel og‘rig‘i va tez-tez umurtqa siqilish siniklari aniqlanadi. Laboratoriyada qondagi Ca²⁺ normal, vitamin D past, suyak rentgenida minerallashish yetishmovchiligi mavjud.Qaysi suyak kasalligi eng mos keladi?",
      a: [
        "Osteoporoz",
        "Osteopeniya",
        "Osteomalatsiya",
        "Osteopetroz",
        "Podagra",
      ],
      correct: 2,
    },
  ],
  mock2: [
    {
      q: "Qorincha devoridagi ko‘ndalang-targ‘il muskullar qaysi anatomik tuzilmalar bilan bog‘langan?",
      a: [
        "Xordae tendinea",
        "Moderator bog‘lama",
        "Qorinchalararo to‘siq",
        "Krista terminalis",
        "Sulkus terminalis",
      ],
      correct: 0,
    },
  ],
  // mock2: [
  //   {
  //     q: "Muskulning nisbatan qimirlamaydigan birikish joyi nima deyiladi?",
  //     a: ["Insertion", "Origin", "Raphe", "Aponevroz"],
  //     correct: 1,
  //   },
  //   {
  //     q: "Fibroz bo‘g‘imlar funksional jihatdan qanday bo‘ladi?",
  //     a: [
  //       "Kam harakatchan",
  //       "To‘liq erkin",
  //       "Ko‘p yo‘nalishli",
  //       "Faqat aylanuvchi",
  //       "Ko'p harakatchan",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Oshiq-moshiq (ginglimus) bo‘g‘imning harakati qanday?",
  //     a: [
  //       "Faqat bukish va yozish",
  //       "Aylana harakat",
  //       "Sirpanish",
  //       "Sirkumduksiya",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Sharsimon (ball-and-socket) bo‘g‘imning xususiyati nima?",
  //     a: [
  //       "Barcha yo‘nalishlarda erkin harakat",
  //       "bukish va yozish",
  //       "sirpanish",
  //       "Faqat aylanma",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Sistemali sirkulatsiyada qon qaysi tomir orqali chiqadi?",
  //     a: [
  //       "Aorta orqali chiqadi",
  //       "Pulmonar vena orqali",
  //       "Yuqori kovak orqali",
  //       "Pastki kovak orqali",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Yurak faoliyatini qaysi sistema boshqaradi?",
  //     a: [
  //       "Avtonom nerv sistemasi",
  //       "Somatik nerv sistemasi",
  //       "Markaziy nerv sistemasi",
  //       "Periferik nerv sistemasi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Asosiy absorbsiya qayerda kechadi?",
  //     a: [
  //       "Ingichka ichakda",
  //       "Yo‘g‘on ichakda",
  //       "Oshqozon ichida",
  //       "Og‘iz bo‘shlig‘ida",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Gaz almashinuvi qayerda sodir bo‘ladi?",
  //     a: [
  //       "Alveola devorida",
  //       "Bronx ichida",
  //       "Hiqqildoq qismida",
  //       "Kekirdak bo‘shlig‘ida",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Epidermis qaysi qatlam?",
  //     a: [
  //       "Yuzaki epiteliy",
  //       "Chuqur biriktiruvchi",
  //       "Yog‘li qatlam",
  //       "Muskulli qatlam",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Tirnoqlar qaysi to‘qimadan hosil?",
  //     a: [
  //       "Epidermis qalinlashuvi",
  //       "Dermis tolalari",
  //       "Gipodermis yog‘i",
  //       "Muskul tolasi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Pulmonar sirkulatsiyaning asosiy vazifasi nima?",
  //     a: [
  //       "Gaz almashinuv vazifasi",
  //       "Oziq moddalar tashish",
  //       "Gormon tarqatish vazifasi",
  //       "Chiqindi chiqarish vazifasi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Periferik nerv sistemasi nimadan tashkil topgan?",
  //     a: [
  //       "Kranial spinal nervlar",
  //       "Bosh orqa miya",
  //       "Po‘stloq yadrolar",
  //       "Trakt qorinchalar",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Funksional jihatdan nerv sistemasi nimaga bo‘linadi?",
  //     a: [
  //       "Somatik viseral tizim",
  //       "Markaziy periferik tizim",
  //       "Sensor motor tizim",
  //       "Ongli reflektor tizim",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Somatik nerv sistemasi nimani boshqaradi?",
  //     a: [
  //       "Ixtiyoriy harakatlar",
  //       "Ichki organlar",
  //       "Bez sekretsiyasi",
  //       "Yurak faoliyati",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Nerv sistemasining struktura birligi nima?",
  //     a: [
  //       "Neyron hujayrasi hisoblanadi",
  //       "Neyrogliya hujayrasi hisoblanadi",
  //       "Gangliy tuzilmasi hisoblanadi",
  //       "Akson tolasi hisoblanadi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Dendritlarning asosiy vazifasi nima?",
  //     a: [
  //       "Impuls olib keladi",
  //       "Impuls olib ketadi",
  //       "Mielin hosil qiladi",
  //       "Sinaps hosil qiladi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Aksonlarning asosiy vazifasi nima?",
  //     a: [
  //       "Impuls olib ketadi",
  //       "Impuls olib keladi",
  //       "Axborot saqlaydi",
  //       "Mielin parchalaydi",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Bipolar neyronlar qaysi sezgilarda uchraydi?",
  //     a: [
  //       "Hid ko‘rish eshitish",
  //       "Og‘riq harorat teginish",
  //       "Bosim propriosepsiya sezgi",
  //       "Motor reflektor harakat",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Multipolar neyronlar qayerda keng tarqalgan?",
  //     a: [
  //       "Markaziy nerv sistemasida",
  //       "Periferik gangliylarda",
  //       "Sensor nervlarda",
  //       "Retseptor hujayralarda",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "MNS da mielinni kim hosil qiladi?",
  //     a: [
  //       "Oligodendrosit hujayralari hosil",
  //       "Shvann hujayralari hosil",
  //       "Astrosit hujayralari hosil",
  //       "Mikrogliya hujayralari hosil",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Orqa miyada kulrang modda qayerda joylashgan?",
  //     a: [
  //       "Markaziy qismda joylashgan",
  //       "Periferik qismda joylashgan",
  //       "Ventral tomonda joylashgan",
  //       "Dorsal tomonda joylashgan",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Kattalarda konus medullaris qayerda tugaydi?",
  //     a: [
  //       "L1 L2 darajasida",
  //       "L3 L4 darajasida",
  //       "T12 L1 darajasida",
  //       "L4 L5 darajasida",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Osteoblastlarning vazifasi qaysi?",
  //     a: [
  //       "Suyakni yemirish",
  //       "Kalsiyni chiqarish",
  //       "Suyak hosil qilish",
  //       "Minerallarni parchalanishi",
  //     ],
  //     correct: 2,
  //   },
  //   {
  //     q: "Qalqon oldi bezi gormoni suyaklarga qanday ta’sir qiladi?",
  //     a: [
  //       "Suyak hosil bo‘lishini kuchaytiradi",
  //       "Suyak yemirilishini kuchaytiradi",
  //       "Osteoid sintezini kamaytiradi",
  //       "Minerallanishni tezlashtiradi",
  //     ],
  //     correct: 1,
  //   },
  //   {
  //     q: "Osteomalatsiyaning asosiy sababi qaysi?",
  //     a: [
  //       "Osteoklast faolligining ortishi",
  //       "Vitamin D yetishmovchiligi",
  //       "Osteoblastlarning ko‘payishi",
  //       "Kollagen ortiqchaligi",
  //     ],
  //     correct: 1,
  //   },
  //   {
  //     q: "Osteoporozda qaysi holat asosiy mexanizm hisoblanadi?",
  //     a: [
  //       "Suyak hosil bo‘lishi ortishi",
  //       "Rezorbsiya va formatsiya tengligi",
  //       "Rezorbsiya formatsiyadan tez kechishi",
  //       "Osteoid ortiqcha sintezi",
  //     ],
  //     correct: 2,
  //   },
  //   {
  //     q: "Osteopetrozda qaysi o‘zgarish kuzatiladi?",
  //     a: [
  //       "Suyak massasining kamayishi",
  //       "Suyak ko‘migi bo‘shlig‘ining torayishi",
  //       "Suyak egiluvchanligining ortishi",
  //       "Diploening kengayishi",
  //     ],
  //     correct: 1,
  //   },
  //   {
  //     q: "Buyrakning asosiy vazifasi?",
  //     a: [
  //       "Qonni tozalash",
  //       "Gormon sekretsiya",
  //       "Qon ishlab",
  //       "Energiya ishlab",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Sharsimon bo‘g‘imga misol qaysi bo‘lishi mumkin?",
  //     a: [
  //       "Yelka va son-chanoq bo‘g‘imlari",
  //       "Bilak bo‘g‘imi",
  //       "Qov simfizi",
  //       "Atlantoaksial bo‘g‘im",
  //     ],
  //     correct: 0,
  //   },
  //   {
  //     q: "Bo‘g‘im, uni harakatlantiruvchi mushak va teri bir nerv bilan innervatsiyalanishini qaysi qonun tushuntiradi?",
  //     a: [
  //       "Hilton qonuni",
  //       "Volf qonuni",
  //       "Laplas qonuni",
  //       "Sherrington qonuni",
  //       "Boyl qonuni",
  //     ],
  //     correct: 0,
  //   },
  // ],
};
